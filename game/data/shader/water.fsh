...
#ifdef SIMPLE
    gl_FragColor = calcLight(vec4(color.rgb, transparency), light_Hemi_Toon.y, false, light_Hemi_Toon.x, vec3(0.0, 0.0, 0.0));
#else    
    gl_FragColor = calcLight(vec4(color.rgb, transparency), light_Hemi_Toon.y, useTexture_1 > 0, light_Hemi_Toon.x, texture2D(texture_1, uv1).xyz * 2.0 - 1.0);
#endif
}
