...
void Magic3D::ImGui_GL_RenderDrawLists(ImDrawData* draw_data)
{
    ...
    GLint last_vertex_array; glGetIntegerv(GL_VERTEX_ARRAY_BINDING_OES, &last_vertex_array);
...
void Magic3D::ImGui_GL_CreateFontsTexture()
{
    ...
    int width, height;
    io.Fonts->GetTexDataAsRGBA32(&pixels, &width, &height);   // Load as RGBA 32-bits for OpenGL demo because it is more likely to be compatible with user's existing shader.
